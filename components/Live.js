...
export default class Live extends Component {
  state = {
    ...
    status: null,
    direction: '',
    bouceValue: new Animated.Value(1),
  }
  componentDidMount () {
  ...
  setLocation = () => {
    Location.watchPositionAsync({
      ...
      const { direction, bouceValue } = this.state

      if(newDirection !== direction) {
          Animated.sequence([
              Animated.timing(bouceValue, {duration: 200, toValue: 1.04}),
    ...
    return (
      <View style={styles.container}>
        <View style={styles.directionContainer}>
          ...
          <Animated.Text 
            style={[styles.direction, {transform: [{scale: bouceValue}] }]}>
            {direction}
          </Animated.Text>
        </View>
